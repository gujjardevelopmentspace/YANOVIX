...
import { motion, useScroll, useTransform, useInView, useSpring, animate, useMotionValue } from "motion/react";
import {
  Bot, Phone, MessagesSquare, Users, Code2, Building2, Workflow, LineChart,
  Cloud, Database, Cpu, Clock, HeartPulse, Home, Scale,
  Landmark, Megaphone, GraduationCap, UserSearch, HardHat, Truck, ArrowRight,
  ArrowUpRight, Check, Sparkles, Globe, Mail, Linkedin, Play, Plus,
  Loader2, MapPin, Menu,
} from "lucide-react";
...
import { CONTACT_EMAIL, CONTACT_MAIL_URL, CONTACT_WHATSAPP_URL, CONTACT_LOCATION, CONTACT_MAPS_URL, CONTACT_MAPS_EMBED, CONTACT_SERVICES, CONTACT_LINKEDIN_URL, sendContactEmail, isContactConfigured } from "@/lib/send-contact";
...
export const Route = createFileRoute("/")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "We build AI agents, voice systems, and custom software for teams tired of demos that never make it to production." },
      { property: "og:title", content: "YANOVIX - AI that actually ships" },
      { property: "og:description", content: "AI agents, voice AI, and custom software - built by senior engineers, owned end to end." },
...
function Counter({ to, suffix = "", duration = 2 }: { to: number; suffix?: string; duration?: number }) {
  ...
  useEffect(() => {
    if (!inView) return;
    const c = animate(0, to, { duration, ease: [0.16, 1, 0.3, 1], onUpdate: (v) => setVal(Math.floor(v)) });
    return () => c.stop();
  }, [inView, to, duration]);
...
function MagneticButton({
  children, tone = "ink", className, type = "button", ...props
...
function SectionHeader({ eyebrow, title, sub, italic }: { eyebrow: string; title: string; sub?: string; italic?: string }) {
  return (
    <div className="mx-auto max-w-3xl text-center">
      <Reveal><Eyebrow>{eyebrow}</Eyebrow></Reveal>
      <h2 className="mt-6 text-4xl leading-[1.05] text-ink sm:text-5xl md:text-6xl">
        <SplitWords text={title} />
        {italic && <> <span className="text-italic-serif text-ember">{italic}</span></>}
      </h2>
      {sub && <Reveal delay={0.15}><p className="mx-auto mt-5 max-w-2xl text-base text-ink/60">{sub}</p></Reveal>}
    </div>
  );
}
...
function CursorGlow() {
  ...
  useEffect(() => {
    const onMove = (e: MouseEvent) => { x.set(e.clientX - 250); y.set(e.clientY - 250); };
  ...
  return (
    ...
    >
      <div className="h-full w-full rounded-full" style={{ background: "radial-gradient(circle, rgba(10,10,10,0.25), transparent 60%)" }} />
    </motion.div>
  );
}
...
function SiteGlow() {
  return (
    <div aria-hidden className="pointer-events-none fixed inset-0 z-0 overflow-hidden">
      ...
      <div className="glow-sage absolute -right-16 top-[40%] h-[380px] w-[380px] animate-glow-pulse" style={{ animationDelay: "2s" }} />
      <div className="glow-ember absolute bottom-10 left-1/3 h-[320px] w-[320px] animate-glow-drift opacity-60" style={{ animationDelay: "5s" }} />
    </div>
  );
}

function VisualFrame({
  src, alt, className, glow = true,
}: { src: string; alt: string; className?: string; glow?: boolean }) {
  return (
    <div className={cn("relative overflow-hidden", className)}>
      {glow && (
        <div
          ...
          style={{ background: "radial-gradient(closest-side, rgba(10,10,10,0.35), transparent 70%)" }}
        />
      )}
      <img
        src={src}
...
function Nav() {
  ...
  const links = [
    ["Services", "services"], ["Industries", "industries"], ["Solutions", "solutions"],
    ["Process", "process"], ["Work", "case-studies"], ["FAQ", "faq"], ["Contact", "contact"],
  ] as const;
      ...
      <div className="container-x">
        <nav
          ...
          className={cn(
            ...
            scrolled ? "border-ink/10 bg-white/80 shadow-[0_10px_40px_-20px_rgba(14,14,12,0.2)] backdrop-blur-xl" : "border-transparent",
          )}
        >
          ...
          <div className="flex items-center gap-2">
            ...
            >
              Let's talk <ArrowUpRight className="h-3.5 w-3.5 transition-transform group-hover:-translate-y-0.5 group-hover:translate-x-0.5" />
            </MagneticButton>
            <Sheet open={open} onOpenChange={setOpen}>
              ...
              <SheetContent side="right" className="flex h-full w-[min(100%,20rem)] flex-col border-ink/10 bg-ivory p-0">
                ...
                <div className="mt-auto border-t border-ink/10 p-5">
                  <MagneticButton tone="ember" className="w-full justify-center" onClick={() => go("contact")}>
...
function Hero() {
  ...
  return (
    <section id="top" className="relative overflow-hidden pt-40 pb-24">
      ...
      <motion.div aria-hidden style={{ y: y2, background: "radial-gradient(closest-side, rgba(10,10,10,0.4), transparent)" }}
        className="pointer-events-none absolute -top-20 right-[10%] h-72 w-72 rounded-full opacity-70 blur-3xl animate-float" />
      <motion.div aria-hidden style={{ y: y1, background: "radial-gradient(closest-side, rgba(82,82,82,0.4), transparent)" }}
        className="pointer-events-none absolute top-1/3 -left-16 h-80 w-80 rounded-full opacity-50 blur-3xl" />

      <div className="container-x relative">
        <div className="mx-auto max-w-4xl text-center">
          <Reveal><Eyebrow>Building with AI since 2019</Eyebrow></Reveal>
          <h1 className="mt-8 text-5xl leading-[1.02] text-ink sm:text-6xl md:text-7xl lg:text-[92px]">
          ...
          <Reveal delay={0.9}>
            <p className="mx-auto mt-8 max-w-2xl text-lg text-ink/60">
              From chatbots and AI SDRs to CRM automations, WhatsApp flows, and custom SaaS - one senior team that builds it and stays past launch.
            </p>
          </Reveal>
          <Reveal delay={1}>
            <div className="mt-10 flex flex-wrap items-center justify-center gap-3">
              <MagneticButton tone="ember" onClick={() => document.getElementById("contact")?.scrollIntoView({ behavior: "smooth" })}>
                Talk to us <ArrowRight className="h-4 w-4" />
              </MagneticButton>
              <MagneticButton tone="ghost" onClick={() => document.getElementById("case-studies")?.scrollIntoView({ behavior: "smooth" })}>
        ...
        <Reveal delay={0.2} className="relative mx-auto mt-16 max-w-5xl">
          <div className="glow-ring overflow-hidden rounded-[28px]">
            <VisualFrame src={visualLiveSystems} alt="YANOVIX live systems" className="aspect-[21/9] min-h-[180px]" />
          </div>
        </Reveal>
            ...
            <div className="card-ink rounded-[22px] p-6">
              ...
              <div className="mt-6 grid grid-cols-2 gap-3 md:grid-cols-4">
                {[
                  ...
                  >
                    <div className="text-[11px] uppercase tracking-widest text-ivory/40">{k.label}</div>
                    <div className="mt-2 text-2xl text-serif text-ivory">
                      {typeof k.val === "number" && k.val < 10 ? `$${k.val}${k.s}` : <><Counter to={k.val as number} suffix={k.s} /></>}
                    </div>
              ...
              <div className="mt-3 grid grid-cols-1 gap-3 md:grid-cols-3">
                <div className="rounded-xl border border-white/5 bg-white/[0.03] p-5 md:col-span-2">
                  ...
                  <svg viewBox="0 0 400 140" className="h-32 w-full">
                    ...
                    <motion.path
                      ...
                      d="M0,110 C60,90 90,40 140,55 C190,70 220,20 270,32 C320,44 360,12 400,18 L400,140 L0,140 Z" fill="url(#lg)"
                    />
                  </svg>
                </div>
                <div className="rounded-xl border border-white/5 bg-white/[0.03] p-5">
                  ...
                  <div className="mt-4 space-y-2.5">
                    {[["Sales AI",84],["Voice AI",92],["Support AI",76],["SDR AI",88]].map(([n,v],i) => (
                      <div key={n as string} className="flex items-center justify-between gap-3 text-xs">
...
const LOGOS = ["Cloudveil", "Northwind", "Meridian", "Helix Health", "Vertex AI", "Ironclad", "Lumen Legal", "Orbit Realty", "Kaizen Bank", "Pulse Media", "Axiom Labs", "Quorum"];
function Trusted() {
    ...
    <section className="border-y border-ink/10 bg-white/40 py-14 backdrop-blur">
      ...
      <div className="relative mt-8 overflow-hidden">
        ...
        <div className="flex w-max animate-marquee gap-14 px-8">
          {row.map((l, i) => (
            <div key={i} className="flex items-center gap-2 whitespace-nowrap text-serif text-2xl text-ink/50 transition hover:text-ink">
              <span className="grid h-6 w-6 place-items-center rounded-md border border-ink/15 text-[10px] text-ink/70">{l[0]}</span>
              {l}
            </div>
          ))}
        </div>
...
function Services() {
  return (
    <section id="services" className="relative py-32">
      ...
      <div className="container-x relative">
        <SectionHeader eyebrow="Services" title="What we actually" italic="build." sub="Voice agents, chatbots, custom SaaS, automations - one senior team that stays with you past launch." />
      ...
      <div className="container-x relative">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <Reveal delay={0.12}>
            <p className="mx-auto mt-5 max-w-xl text-base text-ink/60">
              Same senior builders. Playbooks tuned to how your industry actually operates - and how clients score the outcome.
            </p>
          </Reveal>
        </div>
        ...
        >
          <div
            ...
            style={{ background: `radial-gradient(closest-side, ${featured.accent}99, transparent)` }}
          />
          <div className="relative grid gap-8 p-8 md:grid-cols-[1.2fr_0.8fr] md:p-10 lg:gap-12">
            <div>
              ...
              <p className="mt-4 max-w-xl text-base leading-relaxed text-ivory/60">{featured.short}</p>
              <div className="mt-6 flex flex-wrap gap-2">
                {featured.items.slice(0, 4).map((x) => (
                  <span key={x} className="rounded-full border border-white/15 bg-white/5 px-3 py-1 text-xs text-ivory/65">
                    {x}
                  </span>
                ))}
              </div>
            ...
            <motion.div
              ...
              >
                <div
                  ...
                  style={{ background: `radial-gradient(closest-side, ${ind.accent}66, transparent)` }}
                />
                ...
                <p className="relative mt-2 line-clamp-2 flex-1 text-sm leading-relaxed text-ink/50">{ind.short}</p>
...
const SOLUTIONS = [
  {
    ...
    client: "14-clinic outpatient network, US Midwest",
    desc: "Their front desk could not keep up with evenings and lunch rushes. We shipped voice and web intake that books into the existing schedule, captures insurance basics, and warm-transfers clinical questions - with call logs and access controls their compliance lead signed off on.",
    challenge: "Missed calls and no-shows were eating utilization across 14 sites.",
    tag: "Healthcare",
    stack: ["Twilio Voice", "Calendar sync", "EHR webhook", "PostgreSQL"],
    metric: "~11k hrs",
    metricLabel: "staff time redirected / year",
    metricNote: "Booked visits vs. prior phone-only baseline",
    year: "2025",
    image: solutionHealthcare,
  },
  {
    title: "Speed-to-lead for portal inquiries",
    client: "Regional residential brokerage, 40+ agents",
    desc: "Portal leads sat for hours. An agent layer now replies in seconds, asks budget / timeline / area, scores fit, and pushes warm prospects into HubSpot with a showing link. Closers only get people who want a tour.",
    challenge: "Average first response was 4+ hours; nights and weekends were dead.",
    tag: "Real Estate",
    stack: ["HubSpot", "WhatsApp Cloud", "n8n", "Listing feed API"],
    metric: "2.4x",
    metricLabel: "more showings from same lead volume",
    metricNote: "90-day window vs. prior quarter",
    year: "2025",
    image: solutionRealestate,
  },
  {
    title: "Outbound voice for appointment setting",
    client: "B2B services firm, North America",
    desc: "Reps were burning mornings on dial lists. We built outbound voice that confirms interest, offers two time slots, writes the meeting to Salesforce, and attaches a transcript. Soft dialing caps and opt-out handling were part of go-live, not an afterthought.",
    challenge: "Connect rates were fine; booking from connects was not.",
    tag: "Sales ops",
    stack: ["Realtime voice", "Salesforce", "Deepgram", "Redis queues"],
    metric: "+31%",
    metricLabel: "meetings booked per dial-hour",
    metricNote: "Same list quality; agent vs. human SDR pod",
    year: "2024",
    image: solutionSales,
  },
  {
    title: "CRM copilot for account executives",
    client: "Series C SaaS, ~35 AEs",
    desc: "AEs lived in tabs. We embedded a side panel in Salesforce that drafts account briefs from notes and emails, suggests next steps, and queues follow-ups - with human send. No separate workspace to adopt.",
    challenge: "Prep time before calls was inconsistent; follow-ups slipped after demos.",
    tag: "B2B SaaS",
    stack: ["Salesforce LWC", "Claude API", "MCP tools", "React"],
    metric: "~4 hrs",
    metricLabel: "back per AE each week",
    metricNote: "Time study + activity logs",
    year: "2025",
    image: solutionSaas,
  },
  {
    title: "High-volume resume screen + scheduling",
    client: "Staffing firm placing warehouse & light industrial roles",
    desc: "Recruiters drowned in PDF resumes for the same five role types. Screening scores against a scorecard, shortlists with reasons, and books phone screens into Greenhouse. Recruiters override anything - the model never auto-rejects without a human look on borderline cases.",
    challenge: "Time-to-first-screen was 5-7 days on peak weeks.",
    tag: "Recruiting",
    stack: ["Greenhouse API", "Node + React", "Embeddings", "AWS"],
    metric: "<24h",
    metricLabel: "to first screen on priority reqs",
    metricNote: "Median across top 12 open roles",
    year: "2024",
    image: solutionRecruiting,
  },
  {
    title: "WhatsApp storefront & support desk",
    client: "DTC brand selling into MENA & South Asia",
    desc: "Forms converted poorly where WhatsApp is the default inbox. Catalog browse, cart recovery, order status, and human takeover in one thread - synced to Shopify. Arabic and English templates approved before launch.",
    challenge: "Web checkout drop-off; support lived in personal phones.",
    tag: "E-commerce",
    stack: ["WhatsApp Cloud API", "Shopify", "Make", "Shared inbox"],
    metric: "1.9x",
    metricLabel: "cart recovery vs. email-only",
    metricNote: "Same abandoned-cart cohort definition",
    year: "2025",
    image: solutionEcommerce,
  },
  {
    title: "Order-to-cash exception automation",
    client: "Industrial distributor, SAP + NetSuite estate",
    desc: "Invoice mismatches and approval stalls sat in email. We connected SAP, Salesforce, and NetSuite so routine matches clear automatically, exceptions land in a queue with context, and every action is logged for audit. SSO and role checks from day one.",
    challenge: "AP and sales ops spent days reconciling the same breaks.",
    tag: "Enterprise ops",
    stack: ["SAP BAPI / APIs", "NetSuite", "n8n", "Okta SSO"],
    metric: "-61%",
    metricLabel: "cycle time on matched invoices",
  ...
  },
  {
    ...
    client: "Regional 3PL, 12 warehouses",
    desc: "Support spent the day answering 'where is my freight?' We wired carrier and TMS events into proactive SMS / email updates, an exception queue for SLA slips, and a chat layer that answers routine tracking - claims and damages still go to humans with full history.",
    challenge: "WISMO tickets crowded out real exception work.",
    tag: "Logistics",
    stack: ["TMS webhooks", "Twilio SMS", "n8n", "Shared inbox"],
    metric: "-38%",
    metricLabel: "inbound tracking tickets",
    metricNote: "8 weeks post go-live vs. prior 8 weeks",
    year: "2025",
    image: solutionLogistics,
  },
  {
    title: "Admissions Q&A and application status",
    client: "Private university continuing-ed programs",
    desc: "Every enrollment cycle flooded counselors with the same deadline and document questions. A grounded chat assistant answers from the program catalog and checklist, surfaces application status, and escalates visa / financial-aid edge cases with the thread attached.",
    challenge: "Counselors lost evenings to repeat inbox questions.",
    tag: "Education",
    stack: ["RAG over handbook", "Web chat", "SIS status API", "Slack escalate"],
    metric: "~55%",
    metricLabel: "of routine questions deflected",
    metricNote: "Tagged tickets during peak enrollment month",
    year: "2024",
          ...
          {SOLUTIONS.map((s, i) => (
            ...
            >
              <div className="relative h-44 overflow-hidden">
                ...
                <a
                  href="#contact"
                  className="absolute bottom-4 right-4 grid h-10 w-10 place-items-center rounded-full bg-ember text-ivory shadow-[0_0_24px_rgba(10,10,10,0.55)] transition-transform duration-500 group-hover:rotate-45"
                  aria-label={`Discuss a project like ${s.title}`}
                >
          ...
          <Reveal className="md:col-span-7">
            <div className="group relative overflow-hidden rounded-[28px]">
              ...
              <div className="absolute bottom-5 left-5 right-5 flex flex-wrap items-end justify-between gap-3">
                <div>
                  <div className="text-[11px] uppercase tracking-widest text-ivory/60">Live systems</div>
                  <div className="mt-1 text-serif text-2xl text-ivory md:text-3xl">Voice, agents, ops</div>
                </div>
                <Link
                  to="/"
                  hash="case-studies"
          ...
          <Reveal delay={0.1} className="md:col-span-5">
            <div className="relative flex h-full min-h-[240px] flex-col justify-between overflow-hidden rounded-[28px] border border-ink/10 bg-ink p-8 text-ivory">
              ...
              <div className="relative mt-8 space-y-4">
                <div className="flex items-center gap-3">
                  <div className="overflow-hidden rounded-2xl ring-2 ring-ember/40">
                    <img src={visualMark} alt="" className="h-14 w-14 object-cover" />
                  </div>
...
const CASES = [
  {
    ...
    industry: "Healthcare",
    summary: "Replaced after-hours voicemail with voice and web intake across 14 clinics. Bookings write into the live schedule; clinical edge cases warm-transfer with context. Compliance reviewed logging and access before go-live.",
    kpis: [{ l: "Staff hours redirected", v: "~11k/yr" }, { l: "After-hours coverage", v: "24/7" }, { l: "Go-live", v: "7 wks" }],
  },
  {
    client: "Regional residential brokerage",
    project: "Portal lead response layer",
    industry: "Real Estate",
    summary: "Instant qualification on portal leads, listing-aware answers, and HubSpot handoff so agents only talk to tour-ready buyers. Nights and weekends no longer go cold.",
    kpis: [{ l: "Showings from same volume", v: "2.4x" }, { l: "First response", v: "seconds" }, { l: "Window measured", v: "90 days" }],
  },
  {
    client: "Mid-market litigation practice",
    project: "Matter intake & consult booking",
    industry: "Law",
    summary: "Structured intake by practice area, conflict notes for attorney review, and consult scheduling that stopped reinventing the questionnaire in email every time.",
    kpis: [{ l: "Complete intakes", v: "up" }, { l: "Time to consult", v: "faster" }, { l: "Attorney review", v: "required" }],
  },
  {
    client: "Industrial distributor",
    project: "Order-to-cash exceptions",
    industry: "Enterprise",
    summary: "Routine invoice matches clear across SAP and NetSuite; breaks land in a queue with context and an audit trail. Okta SSO and roles from the first environment.",
    kpis: [{ l: "Matched invoice cycle", v: "-61%" }, { l: "Exception queue", v: "live" }, { l: "Audit trail", v: "full" }],
  },
];
function CaseStudies() {
  return (
    <section id="case-studies" className="relative py-32">
      <div className="container-x">
        <SectionHeader eyebrow="Case Studies" title="Results you can" italic="point to." sub="Engagements we owned from the first workshop through go-live - and stuck around to tune." />
          ...
          {CASES.map((c, i) => (
            ...
            >
              <div className="grid grid-cols-1 gap-8 lg:grid-cols-[1fr_2fr] lg:items-center">
                <div>
                  <div className="text-[11px] uppercase tracking-[0.2em] text-ivory/40">{c.industry}</div>
                  ...
                  <a href="#contact" className="mt-6 inline-flex items-center gap-2 text-sm text-ember transition hover:gap-3">
                ...
                <div className="grid grid-cols-3 gap-3">
                  {c.kpis.map((k, ki) => (
                    ...
                    >
                      ...
                      <div className="mt-2 text-[10px] uppercase tracking-[0.2em] text-ivory/40">{k.l}</div>
                    </motion.div>
                  ))}
                </div>
              </div>
...
const STEPS = [
  { n: "01", t: "Discovery", d: "We sit with your team, map how work actually gets done, and find the bottlenecks." },
  { n: "02", t: "Strategy", d: "A clear plan: what we build first, how we measure success, and how it fits your stack." },
  { n: "03", t: "Design", d: "Flows, prompts, and interfaces sketched so everyone agrees before we write a lot of code." },
  { n: "04", t: "Development", d: "We build in short cycles, demo weekly, and ship with CI/CD from the start." },
  { n: "05", t: "Deployment", d: "Careful rollout, real integrations, load checks, then go-live with eyes on the metrics." },
  { n: "06", t: "Support", d: "We keep watching, fixing, and improving after launch - not vanishing after the invoice." },
];
function Process() {
  const ref = useRef<HTMLDivElement>(null);
    ...
    <section id="process" className="relative bg-white/40 py-32">
      <div className="container-x">
        ...
        <div ref={ref} className="relative mx-auto mt-20 max-w-4xl">
          ...
          <motion.div style={{ height }} className="absolute left-6 top-0 w-px bg-ember md:left-1/2" />
          <div className="space-y-14">
            {STEPS.map((s, i) => (
              <motion.div
                ...
                className={cn("relative grid grid-cols-[48px_1fr] gap-6 md:grid-cols-2", i % 2 && "md:[&>*:first-child]:col-start-2")}
              >
...
const TECH = ["React", "Next.js", "Node.js", "Express", "MongoDB", "PostgreSQL", "TypeScript", "Python", "FastAPI", "LangChain", "OpenAI", "Anthropic", "Google Gemini", "Pinecone", "Supabase", "AWS", "Azure", "Docker", "Kubernetes", "n8n", "Zapier", "Make"];
...
const WHY = [
  { icon: whySecurity, t: "Security that checks out", d: "SSO, roles, encryption, and logs your security team can review." },
  { icon: whyWeMove, t: "We move", d: "You see something working in weeks, not a deck in three months." },
  { icon: whyRealTeam, t: "A real team", d: "Senior people you can Slack - not a rotating cast of juniors." },
  { icon: whyAroundClock, t: "Around-the-clock help", d: "Someone is awake when something breaks - wherever you are." },
  { icon: whyBuiltToGrow, t: "Built to grow", d: "Architecture that does not fall over when usage spikes." },
  { icon: whyNoBlackBoxes, t: "No black boxes", d: "Shared Slack, weekly demos, and dashboards you can open anytime." },
  { icon: whyKnowAi, t: "People who know AI", d: "Not generalists guessing prompts - people who ship this for a living." },
  { icon: whyCloudFluent, t: "Cloud-fluent", d: "AWS, Azure, and GCP experience when your infra needs it." },
];
function Why() {
  return (
    <section className="relative py-32">
...
function Testimonials() {
  return (
    <section className="relative overflow-hidden py-32">
      ...
      <div className="container-x relative">
        <div className="mx-auto max-w-3xl text-center">
          <Reveal><Eyebrow>Client stories</Eyebrow></Reveal>
          <h2 className="mt-6 text-4xl leading-[1.05] text-ink sm:text-5xl md:text-6xl">
          ...
          <Reveal delay={0.15}>
            <p className="mx-auto mt-5 max-w-xl text-base text-ink/60">
              Real operators. Real timelines. The kind of feedback you only get after something actually ships.
            </p>
          </Reveal>
        </div>
        ...
        <div className="mt-16 grid grid-cols-1 gap-5 lg:grid-cols-12 lg:gap-6">
          ...
          >
            ...
            <div
              ...
              style={{ background: `radial-gradient(closest-side, ${TESTIMONIALS[0].accent}88, transparent)` }}
            />
            ...
            >
              ...
              <div className="mt-6 flex items-center justify-between gap-4 border-t border-ink/8 pt-5">
                ...
                <div className="shrink-0 text-right">
                  ...
                  <div className="text-[9px] uppercase tracking-widest text-ink/35">{t.metricLabel}</div>
                </div>
              </div>
        ...
        <Reveal delay={0.2}>
          <p className="mt-10 text-center text-sm text-ink/45">
            Names and roles match the teams we worked with. Numbers come from their post-launch reviews.
          </p>
        </Reveal>
      </div>
    </section>
          ...
          <div className="lg:sticky lg:top-28">
            ...
            <Reveal delay={0.12}>
              <p className="mt-5 max-w-md text-base leading-relaxed text-ink/60">
                Straight answers on timeline, security, ownership, and how we actually work with your team - before you book a call.
              </p>
            </Reveal>
            ...
            <div className="mt-10 grid grid-cols-3 gap-3 border-y border-ink/10 py-6">
              {FAQ_FACTS.map((f, i) => (
                ...
                >
                  ...
                  <div className="mt-1 text-[10px] uppercase tracking-[0.18em] text-ink/40">{f.label}</div>
                </motion.div>
              ))}
            </div>
            ...
            <Accordion type="single" collapsible defaultValue="i-0" className="space-y-3">
              {FAQS.map((f, i) => (
                <motion.div
                  ...
                  viewport={{ once: true, margin: "-40px" }}
                  transition={{ delay: Math.min(i * 0.06, 0.36), duration: 0.65, ease: [0.2, 0.9, 0.2, 1] }}
                >
                  <motion.div whileHover={{ y: -2 }} transition={{ type: "spring", stiffness: 380, damping: 28 }}>
                    <AccordionItem
                      value={`i-${i}`}
                    ...
                    >
                      ...
                      <AccordionContent className="pb-5 pl-0 text-ink/60 sm:pl-12">
                        ...
                        >
                          <p className="max-w-2xl text-sm leading-relaxed sm:text-[0.95rem]">{f.a}</p>
                          {f.points && (
                            <ul className="mt-4 space-y-2 border-t border-ink/8 pt-4">
                              {f.points.map((p, pi) => (
                                <motion.li
                                  ...
                                  transition={{ delay: 0.08 + pi * 0.06, duration: 0.4, ease: [0.2, 0.9, 0.2, 1] }}
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      toast.error("Could not send", {
        ...
        action: {
          label: "Email us",
          onClick: () => { window.open(CONTACT_MAIL_URL, "_blank", "noopener,noreferrer"); },
        },
      });
    } finally {
      setPending(false);
      ...
      <div className="container-x relative">
        <div className="card-ink relative overflow-hidden rounded-[36px] p-8 md:p-14 lg:p-16 glow-ring">
          <div
            ...
            style={{ background: "radial-gradient(closest-side, rgba(10,10,10,0.55), transparent)" }}
          />
            ...
            <div>
              ...
              <Reveal delay={0.5}>
                <p className="mt-6 max-w-md text-ivory/60">
                  Drop a few details. A senior engineer will reply with a practical next step - usually within a business day.
              </p>
            </Reveal>
              <Reveal delay={0.65}>
                ...
                >
                  ...
                  <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <label className="block">
                      <span className="mb-1.5 block text-[11px] uppercase tracking-widest text-ivory/45">Name *</span>
                      <input
                        name="name"
                        required
                        minLength={2}
                    ...
                    <label className="block">
                      <span className="mb-1.5 block text-[11px] uppercase tracking-widest text-ivory/45">Work email *</span>
                      <input
                        name="email"
                        type="email"
                        required
                    ...
                    <label className="block">
                      <span className="mb-1.5 block text-[11px] uppercase tracking-widest text-ivory/45">Company</span>
                      <input
                        name="company"
                    ...
                    <label className="block">
                      <span className="mb-1.5 block text-[11px] uppercase tracking-widest text-ivory/45">Service interest</span>
                      <select
                        name="service"
                        defaultValue=""
                      ...
                      >
                        ...
                        {CONTACT_SERVICES.map((s) => (
                          <option key={s} value={s} className="bg-ink text-ivory">{s}</option>
                        ))}
                      </select>
                    </label>
                  </div>
                  ...
                  <label className="mt-4 block">
                    <span className="mb-1.5 block text-[11px] uppercase tracking-widest text-ivory/45">Project details *</span>
                    <textarea
                      name="message"
                      required
                      minLength={10}
                  ...
                  <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <MagneticButton type="submit" tone="ember" disabled={pending} className="justify-center disabled:opacity-60">
                      {pending ? (
                        <>
    ...
    <footer className="relative border-t border-ink/10 bg-white/40 pt-20">
      <div className="container-x">
        <div className="grid grid-cols-1 gap-12 lg:grid-cols-[1.5fr_2fr_1.5fr]">
          <div>
            ...
            <p className="mt-5 max-w-xs text-serif text-2xl text-ink/80">
              We build software that helps companies <span className="text-italic-serif text-ember">move faster</span> without hiring for every task.
            </p>
          ...
          <div className="grid grid-cols-2 gap-8 sm:grid-cols-4">
            <div>
              ...
              <ul className="mt-4 space-y-2.5">
                <li><FooterLink to="/" hash="contact">Contact</FooterLink></li>
                <li><FooterLink to="/" hash="process">Process</FooterLink></li>
                <li><FooterLink to="/" hash="faq">FAQ</FooterLink></li>
            ...
            <div>
              ...
              <ul className="mt-4 space-y-2.5">
                <li><FooterLink to="/services/$slug" params={{ slug: "ai-agents" }}>AI Agents</FooterLink></li>
                <li><FooterLink to="/services/$slug" params={{ slug: "voice-ai-agents" }}>Voice AI</FooterLink></li>
                <li><FooterLink to="/services/$slug" params={{ slug: "custom-saas" }}>Custom SaaS</FooterLink></li>
                <li><FooterLink to="/services/$slug" params={{ slug: "workflow-automation" }}>Automation</FooterLink></li>
              </ul>
            </div>
            <div>
              <div className="text-[11px] uppercase tracking-widest text-ink/40">Industries</div>
              <ul className="mt-4 space-y-2.5">
                <li><FooterLink to="/industries/$slug" params={{ slug: "healthcare" }}>Healthcare</FooterLink></li>
                <li><FooterLink to="/industries/$slug" params={{ slug: "real-estate" }}>Real Estate</FooterLink></li>
                <li><FooterLink to="/industries/$slug" params={{ slug: "law-firms" }}>Law Firms</FooterLink></li>
                <li><FooterLink to="/industries/$slug" params={{ slug: "logistics" }}>Logistics</FooterLink></li>
              </ul>
            </div>
            <div>
              ...
              <ul className="mt-4 space-y-2.5">
                ...
                <li><FooterLink to="/security">Security</FooterLink></li>
                <li><FooterLink to="/" hash="case-studies">Case Studies</FooterLink></li>
              </ul>
            </div>
          </div>
        ...
        <div className="mt-24 overflow-hidden">
          <div className="text-serif leading-none tracking-tighter text-ink/[0.06]" style={{ fontSize: "clamp(80px, 22vw, 300px)" }}>
            YANOVIX
          </div>
        </div>
        <div className="flex flex-col items-center justify-between gap-4 border-t border-ink/10 py-8 text-xs text-ink/50 md:flex-row">
          ...
          <div className="flex items-center gap-6">
            <Link to="/privacy" className="transition hover:text-ink focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ink/30">Privacy</Link>
            <Link to="/terms" className="transition hover:text-ink focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ink/30">Terms</Link>
            <Link to="/security" className="transition hover:text-ink focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ink/30">Security</Link>
          </div>
        </div>
      </div>
    </footer>
